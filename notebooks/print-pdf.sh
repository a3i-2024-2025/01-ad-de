...
for nb in *.ipynb; do
    ...
    if [ -f "$out" ]; then
        ...
        decktape rise "http://127.0.0.1:8888/notebooks/notebooks/$nb?token=$token" -s 1440x900 "$out" --chrome-arg=--disable-web-security --chrome-arg=--disable-dev-shm-usage --chrome-path="/Applications/Brave Browser.app/Contents/MacOS/Brave Browser"
    fi
done
